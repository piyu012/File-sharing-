...
async def grant_pass(uid:int, hours:int=24):
    ...
    await passes.update_one({"user_id": uid}, {"$set": {"user_id": uid, "validUntil": valid}}, upsert=True)
...
async def mint_token(uid:int, hours:int=24, redeem_window_hours:int=6) -> str:
    ...
    await tokens.insert_one({"token": tok, "user_id": uid, "hours": hours, "createdAt": now, "expiresAt": exp})
    return tok

async def use_token(tok:str, uid:int) -> int | None:
