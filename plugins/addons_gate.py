...
async def start_gate(c,m):
    ...
    await m.reply_text(f"Access locked.
Ad dekhkar token lo: {watch}
Ya token ho to /redeem TOKEN bhejo.")

@Client.on_message(filters.command("redeem"))
async def redeem(c,m):
    parts = m.text.split()
    if len(parts)<2: return await m.reply_text("Format: /redeem TOKEN")
    hours = await use_token(parts[1], m.from_user.id)
    if not hours: return await m.reply_text("Token invalid ya expired.")
