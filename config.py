import os
import logging
from logging.handlers import RotatingFileHandler

BOT_TOKEN = os.environ.get("BOT_TOKEN", "")
API_ID = int(os.environ.get("API_ID", ""))
API_HASH = os.environ.get("API_HASH", "")
OWNER_ID = int(os.environ.get("OWNER_ID", ""))

DB_URL = os.environ.get("DB_URL", "")
DB_NAME = os.environ.get("DB_NAME", "filesharebott")

CHANNEL_ID = int(os.environ.get("CHANNEL_ID", ""))
FORCE_SUB_CHANNEL = int(os.environ.get("FORCE_SUB_CHANNEL", "0"))

FILE_AUTO_DELETE = int(os.getenv("FILE_AUTO_DELETE", "600"))
PORT = os.environ.get("PORT", "8080")
TG_BOT_WORKERS = int(os.environ.get("TG_BOT_WORKERS", "4"))

try:
    ADMINS = [OWNER_ID]
    for x in (os.environ.get("ADMINS", str(OWNER_ID)).split()):
        ADMINS.append(int(x))
except ValueError:
    raise Exception("Your Admins list does not contain valid integers.")

HMAC_SECRET = os.getenv("HMAC_SECRET", "secret")
BASE_URL = os.getenv("BASE_URL", "https://your-app.onrender.com")
BOT_USERNAME = os.getenv("BOT_USERNAME", "")
ADRINO_API = os.getenv("ADRINO_API", "")
SHORTENER_API = os.getenv("SHORTENER_API", "")

CUSTOM_CAPTION = os.environ.get("CUSTOM_CAPTION", None)
PROTECT_CONTENT = True if os.environ.get('PROTECT_CONTENT', "False") == "True" else False
DISABLE_CHANNEL_BUTTON = os.environ.get("DISABLE_CHANNEL_BUTTON", None) == 'True'

START_MSG = os.environ.get("START_MESSAGE", "Hello {first}! I can store private files and share them via special links.")
FORCE_MSG = os.environ.get("FORCE_SUB_MESSAGE", "Hello {first}! You need to join my channel to use me. Please join the channel.")

LOGGER = logging.getLogger(__name__)
logging.basicConfig(
    level=logging.INFO,
    format="[%(asctime)s - %(levelname)s] - %(name)s - %(message)s",
    datefmt='%d-%b-%y %H:%M:%S',
    handlers=[
        RotatingFileHandler("log.txt", maxBytes=50000000, backupCount=10),
        logging.StreamHandler()
    ]
)
